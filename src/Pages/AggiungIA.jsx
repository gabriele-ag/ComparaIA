...
export default function AggiungiIA () {
  ...
  const validateForm = () => {
    ...
    if (!privacyLevelRef.current.value)
      newErrors.privacyLevel = "Il livello di privacy è obbligatorio.";

    const releaseYearValue = Number(releaseYearRef.current.value);
    if (
      !releaseYearRef.current.value.trim() ||
      isNaN(releaseYearValue) ||
      releaseYearValue < 2018 ||
      releaseYearValue > 2025
    ) {
      newErrors.releaseYear =
        "L'anno di rilascio deve essere compreso tra 2018 e 2025";
    }

    const ratingValue = Number(ratingRef.current.value);
    if (
      !ratingRef.current.value.trim() ||
      isNaN(ratingValue) ||
      ratingValue < 0 ||
      ratingValue > 5
    ) {
  ...
  const handleSubmit = async (event) => {
    ...
    const newAI = {
      ...
      website: websiteRef.current.value.startsWith("https://")
        ? websiteRef.current.value.trim()
        : "🤔",
      logoUrl: logoUrlRef.current.value.startsWith("https://")
        ? logoUrlRef.current.value.trim()
        : "🤔",
      ...
      releaseYear: releaseYearRef.current.value
        ...
        : 2025,
      rating: ratingRef.current.value ? Number(ratingRef.current.value) : 0,
    ...
    <section className={styles.sectionAddAi}>
      <div className="container">
        <h1 className={styles.addAiTitle}>
          _aggiungiIA <i className="fa-solid fa-plus"></i>
        </h1>
        <form className={styles.addAiFormFlex} onSubmit={handleSubmit}>
          ...
          <div className={styles.inputBox}>
            ...
            />
            {errors.releaseYear && (
              <p className={styles.errorMsg}>{errors.releaseYear}</p>
            )}
          </div>
          ...
          <div className={styles.inputBox}>
            ...
            />
            {errors.rating && (
              <p className={styles.errorMsg}>{errors.rating}</p>
            )}
          </div>
          ...
          <div className={styles.inputBox}>
            ...
            {errors.privacyLevel && (
              <p className={styles.errorMsg}>{errors.privacyLevel}</p>
            )}
          </div>
          ...
          <button type="submit" className={styles.addAiBtnConfirm}>
            Conferma
          </button>
        </form>
      </div>
